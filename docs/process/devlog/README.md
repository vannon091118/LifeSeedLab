...
| 25 | 24.09.2026 | [T1–T11: Fortschritt, Grenzen, drei Dinge, die kein Test fake-erledigt](2026-09-24_25_t1-t11-konsolidierung.md) | T1–T5, T8, T9 geschlossen; T6/T7/T10/T11 offen mit Begründung; CanvasProbe-`eventId`/`grantedNektar` echter Testhelfer-Defekt |
| 26 | 25.09.2026 | [Drei stille Defekte](2026-09-25_26_drei-stille-defekte.md) | Descriptor-Gewichte zeichengleich (Organ-Tausch war exakt 0 Neuheit, 72 Bruten vermessen); `EFFECT_CHAIN` mit Anteil statt Festbetrag 50; Hook-Exec-Bit in den Index + `.gitattributes` gegen CRLF; 4-Agenten-Review danach: 1 echte Stelle (`markExecutable`-Rückgabe verworfen) behoben, 2 widerlegt (Ketten-Score mit 2 `onEnemyDied`-Belegen gemessen, Index-Nebenwirkung per `update-index -- <pfad>` geprüft); 732/126/42 |
| 27 | 26.09.2026 | [Das Gate, das nur da ist, wenn man ihm folgt](2026-09-26_27-das-gate-das-nur-da-ist-wenn-man-ihm-folgt.md) | `/git-commit` fand nichts zu committen — dafür drei Befunde: Regel 0 beschreibt einen Patch-Bump, den es nicht gibt (0 Treffer über `tools/`+`scripts/`), `VRF000` meldet „Vorsprung intakt" allein aus dem Fehlen der Dateien im Index und blockiert per `VRF001` genau den Bump, den Regel 0 verlangt; `core.hooksPath` ist nicht gesetzt — im Wegwerf-Klon lief ein einfaches `git commit` mit 3 Zeilen Standardausgabe durch (kein shinon/VRF/CHANGELOG, Exit 0). Nichts gefixt, beides Entscheidungen: P-50 (Regel 0 korrigieren oder Bump bauen), P-51 (hooksPath fail-closed setzen). Eigener Fehler dokumentiert: Probe-Commit landete erst im echten Repo, per `--mixed` zurückgedreht |
