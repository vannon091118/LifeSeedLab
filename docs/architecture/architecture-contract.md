...
- **Deterministische Reihenfolge (Befund 20.09.2026):** `String.prototype.localeCompare` ist im Spielcode **verboten** — es kollationiert sprachabhängig (dieselben IDs sortierten unter cs-CZ, da-DK und lt-LT anders als unter en-US). Die kanonische Sortierung ist Bestandteil von Gültigkeiten, die über den Lauf hinaus gelten: `genome_hash` (`src/discovery/chain.ts`), Zustands-Hash (`src/core/hash.ts`) und der Tie-Break des Leitfähigkeits-Dijkstra (`src/simulation/vectorSystem.ts`). Ein sprachabhängiger Hash ist kein Determinismus — dieselbe Klasse Fehler wie `Math.pow`, nur ohne Zahlen. Ersatz ist `compareCodeUnits` aus `src/core/order.ts` (reiner Code-Unit-Vergleich, wie der `sort()`-Default).
  - **Durchsetzung:** Gate-Regel „Deterministische Reihenfolge" + **zweite Regel im selben** Baum-Test `determinism_rule.test.ts` (eine Testdatei, zwei Regeln — kein zweiter Scope-Ort). Die Regel hat **kein `include`, sondern eine Ausschluss-Liste**: neue Ordner sind damit fail-closed erfasst. Ausgenommen ist bewusst nur die Präsentationsschicht (`components/`, `render/`, `observers/`, `i18n.tsx`, `i18n/`) — dort ist sprachrichtige Anzeige-Sortierung legitim und beeinflusst keinen Spielzustand.
