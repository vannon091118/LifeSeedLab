export type Lang = 'de' | 'en';

export const translations = {
  de: {
    'start.title': 'LifeSeedLab',
    'start.subtitle': 'Züchte. Verteidige. Entdecke.',
    'start.tagline': 'PvZ trifft Isaac — deine Kreuzungen sind deine Türme.',
    'start.begin': 'Spiel starten',
    'start.language': 'Sprache',
    'start.hint': 'Alles läuft lokal im Browser. Kein Account nötig.',
    'menu.title': 'Hauptmenü',
    'menu.nektar': 'Nektar',
    'menu.bestWave': 'Beste Welle',
    'menu.runs': 'Läufe',
    'menu.greenhouse': '🌱 Gewächshaus — Samen mischen',
    'menu.greenhouseDesc': 'Kreuze zwei Pflanzen und entdecke neue Varianten. Kostet Nektar.',
    'menu.shop': '🛒 Samen-Shop — Nektar → Samen',
    'menu.shopDesc': 'Kaufe Samen für Nektar. Aussaat passiert im Gewächshaus.',
    'menu.endless': '🌊 Endless',
    'menu.endlessDesc': 'Unendliche Wellen, prozedural generiert. Wie weit kommst du?',
    'menu.pvp': '⚔️ PvP-Board',
    'menu.pvpDesc': 'Triff auf geloggte Runs anderer Spieler. Bald verfügbar.',
    'menu.loadout': 'Mitnahme-Loadout',
    'menu.loadoutFull': 'Loadout voll (max. 4).',
    'menu.take': 'Mitnehmen',
    'menu.leave': 'Ablegen',
    'menu.notOwned': 'Nicht im Bestand.',
    'menu.startRun': 'Run starten',
    'menu.resume': '▶️ Run fortsetzen',
    'menu.resumeDesc': 'Gespeicherter Lauf bei Welle {n} — Pflanzen und Wirtschaft sind erhalten.',
    'menu.collection': 'Sammlung',
    'breed.title': '🧬 Zucht-Labor',
    'breed.parentA': 'Elternteil A',
    'breed.parentB': 'Elternteil B',
    'breed.cross': 'Kreuzen',
    'breed.cost': 'Kosten',
    'breed.offspring': 'Mögliche Nachkommen',
    'breed.keep': 'Nachkommen behalten',
    'breed.new': 'NEU!',
    'breed.chance': 'Chance',
    'breed.collection': 'Deine Sammlung',
    'breed.needTwo': 'Wähle zwei Elternpflanzen',
    'breed.notEnoughNektar': 'Nicht genug Nektar',
    'breed.generation': 'Generation',
    'shop.title': '🛒 Samen-Shop',
    'shop.desc': 'Kaufe Samen für Nektar. Was beim Aussäen KEIMT, ist der Zufall — deterministisch gewürfelt.',
    'greenhouse.title': '🌾 Gewächshaus — Aussaat & Reifung',
    'greenhouse.desc': 'Säe einen Samen aus: Eltern und Kind werden deterministisch gewürfelt. Nach genügend überlebten Wellen ist die Kreuzung bereit.',
    'shop.buy': 'Kaufen',
    'shop.germinate': 'Keimen lassen',
    'shop.germinateHint': 'Ein gekaufter Samen wird sofort zur Pflanze (Bestand) — Keim {n}.',
    'shop.rarity.common': 'Gewöhnlich',
    'shop.rarity.rare': 'Selten',
    'shop.rarity.exotic': 'Exotisch',
    'shop.stash': 'Samen-Vorrat',
    'shop.sow': 'Aussäen (kreuzt zwei Pflanzen — frei, Kosten beim Behalten)',
    'shop.sowEmpty': 'Reifungs-Queue voll ({n}/{m}) — erst abholen!',
    'shop.needTwo': 'Erst 2 Pflanzen nötig (Startbesitz).',
    'shop.parentsGone': 'Elternpflanzen sind nicht mehr vorhanden — die Kreuzung verfällt.',
    'shop.maturing': 'Reift: noch {n} Wellen',
    'shop.ready': 'Bereit zum Keimen!',
    'menu.beetleLab': '🪲 Brutstätte — Käfer züchten',
    'menu.beetleLabDesc': 'Wähle zwei Tiere bewusst und brüte Kampfkäfer für den Einsatz im Feld.',
    'beetle.title': '🪲 Brutstätte — Brüten',
    'beetle.desc': 'Kein Gacha: Du wählst BEIDE Eltern. Ganze Genome verschmelzen — die Brut ähnelt ihren Eltern. Drei Kandidaten pro Brut, deterministisch.',
    'beetle.needTwo': 'Wähle zwei Tiere als Eltern',
    'beetle.notEnoughNektar': 'Nicht genug Nektar',
    'beetle.enqueued': 'Brut angestellt — reift in {n} überlebten Wellen',
    'beetle.breed': 'Brüten',
    'beetle.preview': 'Brutvorschau (3 Kandidaten)',
    'beetle.maturing': 'In Reifung',
    'beetle.ready': 'Bereit — Brutling behalten:',
    'beetle.library': 'Brut-Lager',
    'game.deployBeetle': 'Käfer einsetzen',
    'shop.pending': 'In Reifung ({n}/{m})',
    'gacha.result': 'Der Samen wird:',
    'gacha.parents': 'Eltern gewürfelt:',
    'gacha.keep': 'Übernehmen (kostet 1 Samen)',
    'codex.title': '📖 Öffentlicher Codex',
    'codex.subtitle': 'Jede Kreuzung steht für immer in der Kette — erste Entdeckung gewinnt.',
    'codex.empty': 'Noch keine Entdeckungen. Züchte die erste!',
    'codex.firstBy': 'Erstentdeckung',
    'codex.genome': 'Genom-Hash',
    'codex.share': 'Seed teilen',
    'codex.copied': 'Kopiert!',
    'codex.verify': 'Kette prüfen',
    'codex.valid': 'Kette gültig',
    'codex.invalid': 'Kette ungültig',
    'codex.loadSeed': 'Seed laden',
    'discovery.appended': 'Im Codex verewigt',
    'discovery.duplicate': 'Bereits entdeckt — Erstentdecker bleibt',
    'game.wave': 'Welle',
    'game.waveOf': 'Welle',
    'game.prep': 'VORBEREITUNG — Türme platzieren, dann Welle starten!',
    'game.waveActive': 'Welle',
    'game.enemiesLeft': 'Gegner übrig',
    'game.gameover': 'GAME OVER',
    'game.resumeTap': 'Tippen zum Fortsetzen',
    'game.hint': 'Tippe eine Pflanze unten, ziehe den Geist übers Feld — Tap platziert, ✕ bricht ab. Welt ist Papier, Pflanzen sind lebendig.',
    'hud.journal': 'Forschungsbuch',
    'game.placing': 'Platzieren:',
    'game.clickGrid': '(Klick aufs Raster)',
    'game.startWave': 'Welle starten',
    'game.exitRun': 'Run beenden',
    // B23.1/2: Der Knopf sagt, was gerade gilt — Welle läuft, Countdown oder Warten auf dich.
    'wave.running': 'Welle {n} läuft',
    'wave.startIn': 'Welle startet in {s}s',
    'wave.waitingHint': 'Das Labor wartet auf deine erste Pflanze.',
    // B23.3: Ablehnungsgründe der Platzierung — vorher passierte sichtbar nichts.
    'field.reject.occupied': 'Da wächst schon etwas.',
    'field.reject.on_path': 'Auf dem Weg ist kein Platz.',
    'field.reject.no_inventory': 'Von dieser Pflanze ist keine mehr übrig.',
    'field.reject.no_energy': 'Zu wenig Energie für diese Pflanze.',
    'field.reject.no_energy_tile': 'Zu wenig Energie für dieses Feld.',
    'field.reject.unknown': 'Hier lässt sich gerade nichts setzen.',
    'game.reset': 'Zurücksetzen',
    'game.breed': 'Züchten',
    'hud.energy': 'Energie',
    'hud.lives': 'Leben',
    'hud.nektarEarned': 'Nektar verdient',
    'over.title': 'Run beendet',
    'over.waveReached': 'Erreichte Welle',
    'over.nektarEarned': 'Nektar verdient',
    'over.toMenu': 'Zurück zum Menü',
    'over.score': 'Score',
    'over.retry': 'Neuer Run',
    'debug.title': '⚙ Debug',
    'debug.tick': 'Tick',
    'debug.phase': 'Phase',
    'debug.towers': 'Türme',
    'debug.enemies': 'Gegner',
    'debug.projectiles': 'Projektile',
    'debug.spawnQueue': 'Spawn-Warteschlange',
    'debug.variants': 'Varianten',
    'debug.inventory': 'Inventar',
    'debug.activeTowers': 'Aktive Türme',
    'pvp.comingSoon': 'PvP kommt bald — erst Endless meistern!',
    'common.close': 'Schließen',
    'common.cancel': 'Abbrechen',
    'common.confirm': 'Bestätigen',
    'common.empty': 'Leer',
    'common.back': 'Zurück',
  },
  en: {
    'start.title': 'LifeSeedLab',
    'start.subtitle': 'Breed. Defend. Discover.',
    'start.tagline': 'PvZ meets Isaac — your crossings are your towers.',
    'start.begin': 'Start Game',
    'start.language': 'Language',
    'start.hint': 'Everything runs locally in your browser. No account needed.',
    'menu.title': 'Main Menu',
    'menu.nektar': 'Nectar',
    'menu.bestWave': 'Best Wave',
    'menu.runs': 'Runs',
    'menu.greenhouse': '🌱 Greenhouse — Mix Seeds',
    'menu.greenhouseDesc': 'Cross two plants and discover new variants. Costs Nectar.',
    'menu.shop': '🛒 Seed Shop — Nectar → Seeds',
    'menu.shopDesc': 'Buy seeds with Nectar. Sowing happens in the Greenhouse.',
    'menu.endless': '🌊 Endless',
    'menu.endlessDesc': 'Infinite procedural waves. How far can you get?',
    'menu.pvp': '⚔️ PvP Board',
    'menu.pvpDesc': 'Face logged runs from other players. Coming soon.',
    'menu.loadout': 'Loadout',
    'menu.loadoutFull': 'Loadout full (max. 4).',
    'menu.take': 'Take',
    'menu.leave': 'Leave',
    'menu.notOwned': 'Not owned.',
    'menu.startRun': 'Start Run',
    'menu.resume': '▶️ Resume run',
    'menu.resumeDesc': 'Saved run at wave {n} — plants and economy are kept.',
    'menu.collection': 'Collection',
    'breed.title': '🧬 Breeding Lab',
    'breed.parentA': 'Parent A',
    'breed.parentB': 'Parent B',
    'breed.cross': 'Cross Breed',
    'breed.cost': 'Cost',
    'breed.offspring': 'Potential Offspring',
    'breed.keep': 'Keep Offspring',
    'breed.new': 'NEW!',
    'breed.chance': 'Chance',
    'breed.collection': 'Your Collection',
    'breed.needTwo': 'Pick two parent plants',
    'breed.notEnoughNektar': 'Not enough Nectar',
    'breed.generation': 'Generation',
    'shop.title': '🛒 Seed Shop',
    'shop.desc': 'Buy seeds with Nectar. What sprouts is the roll — deterministic gacha.',
    'greenhouse.title': '🌾 Greenhouse — Sow & Mature',
    'greenhouse.desc': 'Sow a seed: parents and child are rolled deterministically. After enough survived waves the cross is ready.',
    'shop.buy': 'Buy',
    'shop.germinate': 'Germinate',
    'shop.germinateHint': 'A bought seed immediately becomes a plant (stock) — sprout {n}.',
    'shop.rarity.common': 'Common',
    'shop.rarity.rare': 'Rare',
    'shop.rarity.exotic': 'Exotic',
    'shop.stash': 'Seed stash',
    'shop.sow': 'Sow (crosses two plants — free, costs apply on keep)',
    'shop.sowEmpty': 'Queue full ({n}/{m}) — claim first!',
    'shop.needTwo': '2 plants needed first (starter set).',
    'shop.parentsGone': 'Parent plants are no longer available — the cross is lost.',
    'shop.maturing': 'Maturing: {n} waves to go',
    'shop.ready': 'Ready to sprout!',
    'menu.beetleLab': '🪲 Brood Chamber — breed beetles',
    'menu.beetleLabDesc': 'Deliberately pick two specimens and breed battle beetles for field deployment.',
    'beetle.title': '🪲 Brood Chamber — Breeding',
    'beetle.desc': 'No gacha: you pick BOTH parents. Full genomes merge — the brood resembles its parents. Three candidates per brood, deterministic.',
    'beetle.needTwo': 'Pick two parent specimens',
    'beetle.notEnoughNektar': 'Not enough Nectar',
    'beetle.enqueued': 'Brood queued — matures in {n} survived waves',
    'beetle.breed': 'Breed',
    'beetle.preview': 'Brood preview (3 candidates)',
    'beetle.maturing': 'Maturing',
    'beetle.ready': 'Ready — keep a broodling:',
    'beetle.library': 'Brood library',
    'game.deployBeetle': 'Deploy beetle',
    'shop.pending': 'Maturing ({n}/{m})',
    'gacha.result': 'The seed becomes:',
    'gacha.parents': 'Parents rolled:',
    'gacha.keep': 'Keep it (uses 1 seed)',
    'codex.title': '📖 Public Codex',
    'codex.subtitle': 'Every crossing lives forever in the chain — first discovery wins.',
    'codex.empty': 'No discoveries yet. Breed the first one!',
    'codex.firstBy': 'First by',
    'codex.genome': 'Genome hash',
    'codex.share': 'Share seed',
    'codex.copied': 'Copied!',
    'codex.verify': 'Verify chain',
    'codex.valid': 'Chain valid',
    'codex.invalid': 'Chain invalid',
    'codex.loadSeed': 'Load seed',
    'discovery.appended': 'Immortalized in Codex',
    'discovery.duplicate': 'Already discovered — first finder stays',
    'game.wave': 'Wave',
    'game.waveOf': 'Wave',
    'game.prep': 'PREP PHASE — Place towers, then start the wave!',
    'game.waveActive': 'Wave',
    'game.enemiesLeft': 'enemies left',
    'game.gameover': 'GAME OVER',
    'game.resumeTap': 'Tap to Resume',
    'game.hint': 'Tap a plant below, drag the ghost over the field — tap places, ✕ cancels. World is paper, plants are alive.',
    'hud.journal': 'Research Journal',
    'game.placing': 'Placing:',
    'game.clickGrid': '(click grid)',
    'game.startWave': 'Start Wave',
    'game.exitRun': 'Exit Run',
    // B23.1/2: the button says what is true — wave running, countdown, or waiting for you.
    'wave.running': 'Wave {n} running',
    'wave.startIn': 'Wave starts in {s}s',
    'wave.waitingHint': 'The lab is waiting for your first plant.',
    // B23.3: placement rejections — nothing visible used to happen at all.
    'field.reject.occupied': 'Something is already growing there.',
    'field.reject.on_path': 'No room on the path.',
    'field.reject.no_inventory': 'No plant of this kind left.',
    'field.reject.no_energy': 'Not enough energy for this plant.',
    'field.reject.no_energy_tile': 'Not enough energy for this tile.',
    'field.reject.unknown': 'Nothing can be placed here right now.',
    'game.reset': 'Reset',
    'game.breed': 'Breed',
    'hud.energy': 'Energy',
    'hud.lives': 'Lives',
    'hud.nektarEarned': 'Nectar earned',
    'over.title': 'Run Over',
    'over.waveReached': 'Wave Reached',
    'over.nektarEarned': 'Nectar Earned',
    'over.toMenu': 'Back to Menu',
    'over.score': 'Score',
    'over.retry': 'New Run',
    'debug.title': '⚙ Debug',
    'debug.tick': 'Tick',
    'debug.phase': 'Phase',
    'debug.towers': 'Towers',
    'debug.enemies': 'Enemies',
    'debug.projectiles': 'Projectiles',
    'debug.spawnQueue': 'Spawn Queue',
    'debug.variants': 'Variants',
    'debug.inventory': 'Inventory',
    'debug.activeTowers': 'Active Towers',
    'pvp.comingSoon': 'PvP coming soon — master Endless first!',
    'common.close': 'Close',
    'common.cancel': 'Cancel',
    'common.confirm': 'Confirm',
    'common.empty': 'Empty',
    'common.back': 'Back',
  },
} as const;

export type TranslationKey = keyof typeof translations.en;
