...
describe('Zucht-Sprint — MetaSave v8 (`rearingSlots`)', () => {
  ...
  it('kaputte bzw. übergroße Werte werden auf 3..12 geklemmt', () => {
    updateMeta({ rearingSlots: 99 } as never);
    expect(loadMeta().rearingSlots).toBe(12);
    updateMeta({ rearingSlots: 0 } as never);
    expect(loadMeta().rearingSlots).toBe(3);
  });
});
