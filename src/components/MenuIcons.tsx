// Owner: UI (MenuIcons — SVG-Icon-Set der Menüs). LOC ≤ 100.
// 24px, ink stroke, paper fill (B9: keine Emoji-Finalkunst). Reine Präsentation.

export function SproutIcon() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden>
      <path d="M12 21v-8" stroke="#2b2b26" strokeWidth="2" strokeLinecap="round" />
      <path d="M12 13C12 9 9 6 4 6c0 5 3 8 8 7Z" fill="#5a8f4e" stroke="#2b2b26" strokeWidth="2" strokeLinejoin="round" />
      <path d="M12 13c0-4 3-7 8-7 0 5-3 8-8 7Z" fill="#7fb069" stroke="#2b2b26" strokeWidth="2" strokeLinejoin="round" />
    </svg>
  );
}

/** GitHub-Marke der Signatur (B31): kleine gezeichnete Katze in Tusche-Optik — kein Stock-Icon,
 *  kein Emoji (Art-Direction B0). 16px, damit sie das Label nicht dominiert. */
export function GitHubIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden>
      <path
        d="M8.9 3.7 7.5 6.3C5.7 7.1 4.6 8.8 4.6 11c0 1 .3 1.9.8 2.6-.2.7-.3 1.4-.1 2 .6.4 1.5.2 2.3-.2.9.3 1.9.4 3 .4s2.1-.1 3-.4c.8.4 1.7.6 2.3.2.2-.6.1-1.3-.1-2 .5-.7.8-1.6.8-2.6 0-2.2-1.1-3.9-2.9-4.7L15.1 3.7 13 5c-.7-.1-1.4-.1-2 0L8.9 3.7Z"
        fill="#f5efdc" stroke="#2b2b26" strokeWidth="1.7" strokeLinejoin="round"
      />
      <circle cx="9.5" cy="10.8" r="1.15" fill="#2b2b26" />
      <circle cx="14.5" cy="10.8" r="1.15" fill="#2b2b26" />
      <path d="M10 14.2c1.2.85 2.8.85 4 0" stroke="#2b2b26" strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  );
}

export function WaveIcon() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden>
      <path d="M2 14q3-5 6 0t6 0 6 0" stroke="#2b2b26" strokeWidth="2.2" strokeLinecap="round" fill="none" />
      <path d="M2 19q3-5 6 0t6 0 6 0" stroke="#2b2b26" strokeWidth="1.4" strokeLinecap="round" fill="none" opacity="0.5" />
    </svg>
  );
}

export function BookIcon() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden>
      <path d="M4 4h7v16H6a2 2 0 01-2-2V4Z" fill="#d9a441" stroke="#2b2b26" strokeWidth="2" strokeLinejoin="round" />
      <path d="M20 4h-7v16h5a2 2 0 002-2V4Z" fill="#f5efdc" stroke="#2b2b26" strokeWidth="2" strokeLinejoin="round" />
      <path d="M11 4h2v16h-2z" fill="#2b2b26" />
    </svg>
  );
}

export function SeedIcon() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden>
      <ellipse cx="12" cy="13" rx="6" ry="8" fill="#d9a441" stroke="#2b2b26" strokeWidth="2" transform="rotate(-20 12 13)" />
      <path d="M12 5q3-3 6-2-1 4-5 5" fill="#7fb069" stroke="#2b2b26" strokeWidth="1.6" strokeLinejoin="round" />
    </svg>
  );
}

export function BugIcon() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden>
      <ellipse cx="12" cy="14" rx="6.5" ry="7.5" fill="#d9a441" stroke="#2b2b26" strokeWidth="2" />
      <path d="M12 6.5v15" stroke="#2b2b26" strokeWidth="1.6" strokeLinecap="round" />
      <circle cx="12" cy="5.5" r="2.4" fill="#2b2b26" />
      <path d="M9 3.5 10.6 5M15 3.5 13.4 5" stroke="#2b2b26" strokeWidth="1.6" strokeLinecap="round" />
      <path d="M5.8 10.5 3.2 9.2M18.2 10.5 20.8 9.2M5.5 14H2.8M18.5 14h2.7M5.8 17.5l-2.4 1.6M18.2 17.5l2.4 1.6" stroke="#2b2b26" strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  );
}
