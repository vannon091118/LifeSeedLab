...
- [Doku/Prozess] Drei Befunde dokumentiert, **kein Code geändert**: Regel 0 beschreibt einen Patch-Bump der Versionsdateien, den es nicht gibt (`hookScripts()` ruft nur `gate` auf; Suche nach jeder Bump-Form über `tools/`+`scripts/` liefert 0 Treffer), `VRF000` meldet „Vorsprung intakt" allein daraus, dass die Dateien nicht im Index stehen, und `core.hooksPath` ist nicht gesetzt — belegt in einem Wegwerf-Klon, in dem ein einfaches `git commit` mit exakt drei Zeilen Standardausgabe durchlief (kein shinon, kein `VRF`, kein CHANGELOG-Nachweis, Exit 0). Belege: Contract **B39.1–B39.3** in `docs/quality/contracts/process.md`, ROADMAP §3 als **P-50** (Bump fehlt / `VRF001` erzwingt die Umkehrung) und **P-51** (Gate-Zwang lokal inaktiv), Devlog 27. Beides sind Entscheidungen des Eigentümers, keine gefixten Defekte.
...
- [Sim/Vector] Die Vektor-Engine bekommt eine schmale Beobachtungsgrenze (`SimObservation`, `SimulationRoot.getObservation`) statt `structuredClone` des Gesamt-Zustands, fail-closed Deposits gegen `NaN`/`Infinity`, eine gekapselte `VectorTraceState`-Projektion und 72 entfallene gebackene Sin/Cos-Literale — Bewegung bleibt radial und deterministisch, ohne transitive Funktionen in `src/`. Belege: `src/simulation/vectorSystem.ts`, `src/simulation/vectorAttractor.ts`, `src/simulation/state.ts`, `src/simulation/root.ts`, `src/testing/vectorFixture.ts`, `src/simulation/vectorSystem.test.ts` (36 Quell-Kombinationen) und `src/simulation/vectorDeterminism.test.ts`.

- [Shinon/Doku] Die Gate-Dokumentation, CI-Referenz und Sprint-Verifikation zeigen auf `hook-entry.mjs` und beschreiben Staging-vor-Gate, read-only Dry-Run und den single-owner Push-Vertrag. Belege: `AGENTS.md`, `docs/setup/script-readme.md`, `docs/process/plan-sprint-maze-agent.md`, `tools/README.md` und `.github/workflows/ci.yml`.

- [Sim/Vector] Die Kosten- und Cachefaehigkeit des GameViews wird gegen einen echten Closure-Stale-Error gewechselt: `toggleFx` las den FX-Schalter vorher funktional, wodurch der Runtime-Toggle einen veralteten `fxOn`-Wert bekam und das Umschalten unter Last aus dem Takt geriet. Belege: `src/components/GameView.tsx` und `tests/visual_probe.spec.ts`.

- [Test-Lane] Die Test-Lane trennt Projekt- und Tooling-Änderungen, eskaliert fehlende Zuordnung auf die jeweilige Suite und bewertet Voll-Läufe mit Git-Revision, Load-Band und versioniertem Zustand statt mit einem wanduhrabhängigen Persistentitätsmarker. Belege: `scripts/test-lane.mjs`, `scripts/test-lane-verdict.mjs`, `tools/vitest.config.ts` und `tools/shinon/tests/test-lane-verdict.test.ts`.
