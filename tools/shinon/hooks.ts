 ...
 * Nachrichtenregel wie der Komponist, post-commit stößt die Push-Stufe an (abschaltbar über
 * `push.autoAfterCommit`). Kein `npx`-Download, kein ts-node: Node führt die Module direkt aus.
 */
...
export function hookScripts(): Record<string, string> {
  ...
  return {
    'pre-commit':
      `${header}${root}` +
      '# Gate-Stufe: Modulgrenzen, Constraints, Typecheck, Tests\nexec node "$SHINON_ROOT/tools/shinon/cli.ts" gate --phase=pre-commit --quiet\n',
    'commit-msg':
      `${header}${root}` +
      '# Nachrichtenregel: genau diese Nachricht wird der Komponist commiten\nexec node "$SHINON_ROOT/tools/shinon/cli.ts" message --file="$1" --quiet\n',
    'post-commit':
      `${header}${root}` +
      '# Push-Stufe: automatisch nach grünem Gate (push.autoAfterCommit)\nexec node "$SHINON_ROOT/tools/shinon/cli.ts" push --auto --quiet\n',
  };
}
