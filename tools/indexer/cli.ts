...
const INDEX_DIR = path.join(REPO_ROOT, '.index');
...
function extractionContext(program: ts.Program, repoFiles: string[]): ExtractContext {
...
export function buildIndex(): Index {
  ...
  const ctx = extractionContext(program, repoFiles);
  ...
  // vollstaendige Repo-Inventar — getrennt vom Program, damit dort keine
...
function writeIndex(index: Index): string[] {
  const written: string[] = [];
  fs.mkdirSync(INDEX_DIR, { recursive: true });

  const jsonPath = path.join(INDEX_DIR, 'index.json');
  fs.writeFileSync(jsonPath, renderIndexJson(index), 'utf8');
  written.push(jsonPath);

  const rootIndex = path.join(REPO_ROOT, 'INDEX.md');
  fs.writeFileSync(rootIndex, renderRootIndex(index, repositorySections()), 'utf8');
  written.push(rootIndex);

  for (const module of index.modules) {
    const moduleIndex = path.join(REPO_ROOT, module.path, 'INDEX.md');
    fs.writeFileSync(moduleIndex, renderModuleIndex(module, index, []), 'utf8');
...
function checkIndex(index: Index): number {
  const expected = new Map<string, string>();
  expected.set(path.join(INDEX_DIR, 'index.json'), renderIndexJson(index));
  expected.set(path.join(REPO_ROOT, 'INDEX.md'), renderRootIndex(index, repositorySections()));
  for (const module of index.modules) {
    expected.set(
      path.join(REPO_ROOT, module.path, 'INDEX.md'),
  ...
  for (const [file, content] of expected) {
    ...
    if (actual !== content) {
      problems.push(`veraltet: ${toRepoRelative(REPO_ROOT, file)} (index:build ausführen)`);
    }
  }
